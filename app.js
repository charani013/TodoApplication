  ...
  switch (true) {
    case hasStatusProperty(request.query):
      ...
      WHERE
        ...
        response.send(data.map((eachItem) => todoApiResult(eachItem)));
      } else {
        response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(todoApiResult(todo));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    response.send(result.map((eachItem) => todoApiResult(eachItem)));
  } else {
    response.status(400);
